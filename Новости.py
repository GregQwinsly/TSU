...
try:
    ...
    for data in NewsBlock:
        if data.find('time', class_='date') is not None:
            ...
            print(Tstr)
            # запись в лог новостей не старее 4 часов от стартового времени времени, которые содержат упомиинание Армении или Азербайджана
            if DTN > TimeInt and (Tstr.find('Армен')>0 or Tstr.find('Азербайдж')>0):
